// Feriados nacionais (fixos + móveis) e verificação de dia útil (segunda a sexta),
// já que a empresa não trabalha aos finais de semana nem em feriados.
...
/**
 * Feriados nacionais obrigatórios (CLT) + pontos facultativos amplamente observados
 * pelas empresas (Carnaval, Sexta-feira Santa, Corpus Christi), calculados para o ano informado.
 */
function feriadosNacionais(ano: number): Set<string> {
  ...
  const moveis = [
    formatarISO(somarDias(pascoa, -48)), // Carnaval (segunda-feira)
    formatarISO(somarDias(pascoa, -47)), // Carnaval (terça-feira)
    formatarISO(somarDias(pascoa, -2)), // Sexta-feira Santa
    formatarISO(somarDias(pascoa, 60)), // Corpus Christi
  ];
