...
export default async function ReservasViagemPage() {
  ...
  const response = await apiFetch("/api/solicitacoes-viagem?status=aprovado");

  if (!response.ok) {
    return (
      <div className="mx-auto max-w-5xl px-4 py-8">
        <p className="text-sm text-red-600 dark:text-red-400">Não foi possível carregar as solicitações aprovadas.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Reservas pendentes</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Solicitações aprovadas aguardando reserva/compra</p>
        </div>
      ...
      {solicitacoes.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 p-10 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
          Nenhuma solicitação aprovada aguardando reserva no momento.
        </div>
      ) : (
