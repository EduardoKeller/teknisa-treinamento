...
export default function AdminCentrosCustoPage() {
  ...
  return (
    <div>
      <form onSubmit={criarCentro} className="mb-6 flex gap-3 rounded-lg border border-dashed border-gray-300 p-4">
        <input
          type="text"
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {centros.map((centro) => (
              <tr key={centro.id}>
                <td className="px-4 py-2 text-gray-900">{centro.nome}</td>
                <td className="px-4 py-2">
                  <span
                    className={`rounded-full px-2 py-1 text-xs font-medium ${
                      centro.ativo ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                    }`}
                  >
