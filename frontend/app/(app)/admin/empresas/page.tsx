...
export default function AdminEmpresasPage() {
  ...
  return (
    <div>
      <form onSubmit={criarEmpresa} className="mb-6 flex gap-3 rounded-lg border border-dashed border-gray-300 p-4">
        <input
          type="text"
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {empresas.map((empresa) => (
              <tr key={empresa.id}>
                <td className="px-4 py-2 text-gray-900">{empresa.nome}</td>
                <td className="px-4 py-2">
                  <span
                    className={`rounded-full px-2 py-1 text-xs font-medium ${
                      empresa.ativa ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                    }`}
                  >
